...
async function bootstrap() {
  ...
  try {
    const app = await NestFactory.create(AppModule, {logger: ['error', 'warn', 'log', 'debug', 'verbose', 'fatal']});
    ...
    SwaggerModule.setup('docs', app, documentFactory, {customSiteTitle: 'Turkana kafe API Docs'});
    ...
    const logger = new Logger('Bootstrap');
    logger.error('No se pudo conectar a la base de datos o iniciar la aplicación.', error);
    process.exit(1);
  }
}
bootstrap();